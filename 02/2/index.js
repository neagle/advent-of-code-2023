...
games 2-5 it was 12, 1560, 630, and 36, respectively. Adding up these five
...
import { parseGame, minimumSet } from "../utils.js";

const input = await Deno.readTextFile("../1/input");
let lines = input.split("\n");
const games = lines.map(parseGame);

const minimumSets = games.map(minimumSet);

const setPowers = minimumSets.map((set) =>
  ["red", "green", "blue"].reduce((power, color) => power * set[color], 1)
);
