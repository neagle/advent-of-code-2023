...
function isSymbolAdjacent({ start, end }, index) {
  let foundSymbol = false;
  const chars = [];
  const line = lines[index];

  // Check the preceding and following chars in the same line
  chars.push(line.charAt(start - 1));
  chars.push(line.charAt(end));

  // Check the previous and next lines
  const previousLine = lines[index - 1];
  const nextLine = lines[index + 1];

  [previousLine, nextLine].forEach((line) => {
    if (line) {
      // Check each char starting and ending with diagonals
      for (let i = start - 1; i <= end && !foundSymbol; i += 1) {
  ...
  // Check the set of chars for symbols
  for (let i = 0; i < chars.length && !foundSymbol; i += 1) {
    foundSymbol = isSymbol(chars[i]);
  }
...
const engineParts = lines
  .map((line, i) => {
    ...
    for (const match of matches) {
      ...
      if (isSymbolAdjacent({ start, end }, i)) {
...
console.log("engineParts", engineParts);

const enginePartsSum = engineParts.reduce((sum, part) => sum + part, 0);
console.log("enginePartsSum", enginePartsSum);
