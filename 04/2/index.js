...
function parseCard(str) {
  const card = {
    ...str.match(/Card\s+\d+: (?<winners>[\d\s]+) \| (?<numbers>[\d\s]+)/)
      ?.groups,
  };

  ["winners", "numbers"].forEach((key) => {
    card[key] = card[key]
      .split(" ")
      .filter(Boolean)
...
function cardCopier(card, i) {
  const value = card.numbers.reduce(
    (total, num) => total + (total = card.winners.includes(num) ? 1 : 0),
    0
  );

  // Go forward ${value} cards and increase the number of copies by how many
  // copies THIS card currently has.
  for (let j = i + 1; j <= i + value; j += 1) {
    if (cards[j]) {
...
// Iterate through the list of cards and add copies according to the value of
// each card
cards.forEach(cardCopier);
