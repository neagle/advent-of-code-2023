...
remaining 4 milliseconds of travel time, the boat will have gone 12 millimeters.

- Hold the button for 4 milliseconds. After its remaining 3 milliseconds of
travel time, the boat will have gone 12 millimeters.  - Hold the button for 5
milliseconds, causing the boat to travel a total of 10 millimeters.  - Hold the
button for 6 milliseconds, causing the boat to travel a total of 6 millimeters.
...
const [times, records] = Object.values(
  input.match(/Time:\s+(?<times>[\d\s]+)\nDistance:\s+(?<distances>[\d\s]+)/)
    .groups
).map((value) => value.split(/\s+/).map(Number));

// Combine time/records into one array
const races = times.map((time, i) => [time, records[i]]);

// Use the quadratic formula to find the two values of the quadratic equation
// created by knowing the race duration and the record time that could've been
// used.
//
// Intuitively, you can see just from the sample in the problem that the results
// follow a curve... charging too little gets diminished results, but then past
// a certain point charging too long loses results. That shows we've got a
// curve, with an idea spot at its peak.
//
// That means that if we know only a record time AND that it wasn't the single,
// peak, ideal, we have two possible answers. That's what the quadratic formula
// solves.
//
// Full disclosure: I asked ChatGPT to help me reformulate this into the
// quadratic equation since my algebra is VERY rusty.
// Me: "How can I use the quadratic equation to find the two solutions for 9 =
// (7 - x) * x?"
// https://chat.openai.com/share/360455af-516c-4bb7-8e31-853118032a64
...
const winners = races.map(winningTimes);

const winnersProduct = winners.reduce(
  (product, winners) => product * winners,
  1
);

console.log("winnersProduct", winnersProduct);
