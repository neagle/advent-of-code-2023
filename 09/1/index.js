...
function extrapolateNextValue(analysis) {
  let value = analysis.at(-1).at(-1);

  for (let i = analysis.length - 2; i >= 0; i -= 1) {
    value += analysis[i].at(-1);
  }
...
const extrapolatedValues = analyzedHistories.map(extrapolateNextValue);
